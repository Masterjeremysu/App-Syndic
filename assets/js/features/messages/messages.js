  ...
  page.innerHTML = `
    <div class="msg-inner-tabs" id="msg-inner-tabs">
      <button class="msg-inner-tab ${_msgState.activeMobileTab === 'feed' ? 'active' : ''}"
        ...
        <span class="msg-inner-tab-ico">🏘️</span>
        <span>Fil</span>
      </button>
      <button class="msg-inner-tab ${_msgState.activeMobileTab === 'channels' ? 'active' : ''}"
        onclick="switchMobileTab('channels')" id="msg-tab-channels">
        <span class="msg-inner-tab-ico">💬</span>
        <span>Canaux</span>
        <span class="msg-inner-tab-badge" id="tab-badge-channels" style="display:none;"></span>
      </button>
      <button class="msg-inner-tab ${_msgState.activeMobileTab === 'dms' ? 'active' : ''}"
        onclick="switchMobileTab('dms')" id="msg-tab-dms">
        <span class="msg-inner-tab-ico">🔒</span>
        <span>DMs</span>
    ...
    <div class="msg-layout" id="msg-layout">
      <div class="msg-sidebar" id="msg-sidebar">
        ...
        <div class="msg-sidebar-scroll">
          ...
          <div class="msg-chan-item ${_msgState.activeChanType==='feed'?'active':''}" id="chan-feed" onclick="openFeed()">
            ...
            <div class="msg-chan-name">Fil du quartier</div>
          </div>
          <div class="msg-section-label">Canaux</div>
          <div id="chan-list-groups"></div>
          <div class="msg-section-label">Messages privés</div>
      ...
      <div class="msg-main" id="msg-main">
        <div class="chat-empty" id="msg-welcome">
          ...
          <div style="font-size:13px;color:var(--text-3);">Fil du quartier, canaux et messages privés</div>
        </div>
      </div>
    </div>`;
...
function _updateMobileTabBadges() {
  ...
  if (chanBadge) {
    chanBadge.textContent = chanUnread > 9 ? '9+' : chanUnread;
  ...
  if (dmBadge) {
    dmBadge.textContent = dmUnread > 9 ? '9+' : dmUnread;
...
function renderSidebarGroups() {
  ...
  el.innerHTML = sorted.map(c => {
    ...
    return `<div class="msg-chan-item ${isActive?'active':''}" id="chan-${c.id}" onclick="openConv('${c.id}')">
      ...
      <div class="msg-chan-name">${escHtml(titre)}${isMaTour ? ' <span style="font-size:10px;color:var(--accent);">Ma tour</span>' : ''}</div>
...
function setFeedComposeCategory(cat) {
  ...
  if (ph) {
    const hints = {
      ...
      evenements: 'Fête des voisins, AG conviviale…',
...
function openFeedComposeModal() {
  ...
  const composeCatHtml = feedComposeCatsForUser().map(c => {
    ...
    return `<button type="button" class="feed-modal-cat ${on ? 'active' : ''}" data-cat="${c.id}"
      onclick="setFeedComposeCategory('${c.id}')">${c.emoji} ${escHtml(c.label)}</button>`;
  }).join('');
  ...
  overlay.innerHTML = `
    <div class="feed-compose-modal" id="feed-compose-modal">
      <div class="feed-compose-modal-header">
        <span class="feed-compose-modal-title">Publier dans le fil</span>
      ...
      <div class="feed-compose-modal-body">
        ...
        <div style="display:flex;gap:10px;align-items:flex-start;">
          ...
          <textarea class="input" id="feed-compose-modal-input"
            placeholder="${cat === 'panneau' ? 'Message officiel…' : 'Partager avec vos voisins…'}"
            rows="4"
            style="flex:1;resize:none;min-height:100px;border-radius:12px;padding:10px 13px;"
      ...
      <div class="feed-compose-modal-footer">
        <button type="button" class="btn btn-secondary" onclick="closeFeedComposeModal()">Annuler</button>
        <button type="button" class="btn btn-primary" onclick="publishFeedPost()">Publier</button>
      </div>
    </div>`;
  ...
  if (draft) {
    const ta = $('feed-compose-modal-input');
    if (ta) { ta.value = draft; ta.style.height = 'auto'; ta.style.height = Math.min(ta.scrollHeight, 200) + 'px'; }
  }
}
...
async function openFeed() {
  ...
  main.innerHTML = `
    <div class="msg-chan-header feed-header-compact" style="flex-shrink:0;">
      ...
      <div>
        <div class="msg-chan-title">Fil du quartier</div>
        <div class="msg-chan-desc">Panneau d'affichage & échanges entre voisins</div>
      </div>
    </div>
    ...
    <div class="feed-compose-bar" style="flex-shrink:0;">
      ...
      <button type="button" class="btn btn-primary btn-sm" style="border-radius:18px;white-space:nowrap;" onclick="openFeedComposeModal()">
        + Publier
      </button>
    </div>
      ...
      <div class="feed-thread-pane" id="feed-thread-pane">
        <div class="feed-thread-empty" id="feed-thread-empty">
          ...
          <div style="font-size:13px;color:var(--text-3);line-height:1.5;">
            Tapez sur "Commenter" pour voir la conversation.
          </div>
        </div>
      </div>
    </div>`;
...
function renderFeed() {
  ...
  if (!filtered.length) {
    ...
    el.innerHTML = `<div class="feed-empty-cat">
      ...
      <div class="feed-empty-cat-title">${f === 'tout' ? 'Soyez le premier à poster !' : `Pas encore dans « ${escHtml(cm.label)} »`}</div>
      <div class="feed-empty-cat-desc">${f === 'tout'
        ? 'Tapez sur "+ Publier" pour partager avec vos voisins.'
        : 'Essayez une autre rubrique ou publiez ici.'}</div>
      <button class="btn btn-primary btn-sm" style="margin-top:12px;" onclick="openFeedComposeModal()">+ Publier</button>
    </div>`;
    return;
  }
    ...
    html += `<section class="feed-board-section" aria-label="Panneau — messages épinglés">
      <header class="feed-board-head">
        ...
        <div>
          ...
          <div class="feed-board-head-sub">À lire en priorité</div>
        </div>
      </header>
  ...
  if (rest.length) {
    if (showBoard) html += `<div class="feed-timeline-label"><span>Fil du quartier</span></div>`;
    html += rest.map(p => renderFeedPost(p)).join('');
  } else if (showBoard) {
    html += `<div class="feed-timeline-end">— Rien d'autre pour le moment —</div>`;
  }
...
function renderFeedPost(p) {
  ...
  const pinBtn = canManageAnnonces() && p.type === 'post'
    ? `<button type="button" class="btn btn-ghost btn-sm feed-pin-btn" title="${p.epingle ? 'Retirer du panneau' : 'Épingler'}" onclick="event.preventDefault();toggleFeedPin('${p.id}')">${p.epingle ? '📌' : '📍'}<span class="feed-pin-lbl">${p.epingle ? 'Épinglé' : 'Épingler'}</span></button>`
    : '';
...
async function publishFeedPost() {
  ...
  if (!feedComposeCatsForUser().some(c => c.id === cat)) {
    cat = (feedComposeCatsForUser()[0] || { id: 'entraide' }).id;
  }

  const titreInp = $('feed-titre-panneau-modal') || $('feed-titre-panneau');
...
function startFeedRealtime() {
  ...
  _msgState.feedChannel = sb.channel('feed-global')
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'feed_posts' }, async payload => {
      ...
      if (p.type === 'comment') {
        ...
        if (isThreadActive) {
          const { data: prof } = await sb.from('profiles')
            .select('id,prenom,nom,email').eq('id', p.auteur_id).single();
      ...
      else toast('🏘️ Nouveau message sur le fil du quartier', 'ok');
    })
...
function pickFeedEmoji(e) {
  ...
  picker.innerHTML = EMOJIS.map(e =>
    `<button class="emoji-opt" onclick="insertFeedEmoji('${e}')">${e}</button>`
  ).join('');
...
function insertFeedEmoji(emoji) {
  const input = $('feed-compose-modal-input') || $('feed-input');
  if (input) {
