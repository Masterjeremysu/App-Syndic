...
function getTicketDraft() {
  try { const raw = localStorage.getItem(TICKET_DRAFT_KEY); return raw ? JSON.parse(raw) : null; }
  catch { return null; }
}
...
function addPhotoThumb(src, idx) {
  ...
  thumb.id = `thumb-${idx}`;
  thumb.innerHTML = `<img src="${src}" alt="Photo ${idx+1}" onclick="window.open('${src}','_blank')"><button class="photo-thumb-del" onclick="removePhoto(${idx})" title="Supprimer">×</button>`;
...
async function submitTicket() {
  ...
  if (filesToUpload.length > 0) {
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      if (!upErr) {
        ...
        console.warn('[upload photo]', upErr.message);
      }
    }
    if (photos_urls.length > 0) {
      await sb.from('tickets').update({ photo_url: photos_urls[0], photos_urls }).eq('id', data.id);
  ...
  // ✅ Notifs in-app pour les critiques
  if (urgencySelected === 'critique') {
    await notifNouveauTicketCritique(data);
  }
  ...
  await pushNotif(
    ...
    urgencySelected === 'critique' ? 'ticket_critique' : 'nouveau_ticket',
    data.id
  );
